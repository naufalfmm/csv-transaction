...
func (so *selectPostgreOrm) With(name string, query orm.QueryAppender) orm.Select {
	...
	if ok {
		so.sq = so.sq.With(name, sqa.sq)
	}

	vqa, ok := query.(*valuesPostgreOrm)
	if ok {
		so.sq = so.sq.With(name, vqa.vq)
	}
...
func (so *selectPostgreOrm) Copy() orm.Select {
	return &selectPostgreOrm{
		sq: *&so.sq,
	}
}
...
func (so *selectPostgreOrm) AppendColumns(fmter schema.Formatter, b []byte) ([]byte, error) {
	return nil, nil
}
...
func (vo *valuesPostgreOrm) AppendColumns(fmter schema.Formatter, b []byte) ([]byte, error) {
	return vo.vq.AppendColumns(fmter, b)
}
